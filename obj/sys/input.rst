...
Hexadecimal [16-Bits]
   ...
   2472 00 00                48     .dw 0
                             49 
   2474                      50 sys_input_player_actions::
   2474 04 04 6F 1D          51     .dw Key_O,      sys_input_player_left
   2478 03 08 98 1D          52     .dw Key_P,      sys_input_player_right
   247C 05 80 11 1E          53     .dw Key_Space,  sys_input_player_jump
   2480 08 08 C0 1D          54     .dw Key_Q,      sys_input_player_up
   2484 08 20 E9 1D          55     .dw Key_A,      sys_input_player_down
...
Hexadecimal [16-Bits]
   ...
   1CF4                      79 sys_input_clean_buffer::
   1CF4 CD 29 21      [17]   80     call cpct_isAnyKeyPressed_asm
   1CF7 20 FB         [12]   81     jr nz, sys_input_clean_buffer
   1CF9 C9            [10]   82     ret
                             83 
                             84 ;;-----------------------------------------------------------------
                             85 ;;
                             ...
                             91 ;;  Modified: 
                             92 ;;
   1CFA                      93 sys_input_wait4anykey::
   1CFA 21 00 00      [10]   94     ld hl, #0
   1CFD                      95 _siw_loop:
   1CFD E5            [11]   96     push hl
   1CFE CD 29 21      [17]   97     call cpct_isAnyKeyPressed_asm
   1D01 B7            [ 4]   98     or a
   1D02 E1            [10]   99     pop hl
   1D03 23            [ 6]  100     inc hl
   1D04 28 F7         [12]  101     jr z, _siw_loop
   1D06 E5            [11]  102     push hl
   1D07 CD F4 1C      [17]  103     call sys_input_clean_buffer
   1D0A E1            [10]  104     pop hl
   1D0B C9            [10]  105     ret
                            106 
                            107 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            108 ;; DESCRIPTION
                            ...
                            122 ;;
   1D0C                     123 sys_input_getKeyPressed::
   1D0C 21 73 20      [10]  124     ld hl, #_cpct_keyboardStatusBuffer
   1D0F AF            [ 4]  125     xor a                           ;; A = 0
                            126 
   1D10                     127 _kp_loop:
   1D10 FE 0A         [ 7]  128     cp #BUFFER_SIZE
   1D12 28 14         [12]  129     jr z, _kp_endLoop               ;; Check counter value. End if its 0
   1D14 32 24 1D      [13]  130     ld (_size_counter), a
                            131 
   1D17 7E            [ 7]  132     ld a, (hl)                      ;; Load byte from the buffer
   1D18 EE FF         [ 7]  133     xor #ZERO_KEYS_ACTIVATED        ;; Inverts bytes
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 108.
Hexadecimal [16-Bits]



   1D1A 28 06         [12]  134     jr z, _no_key_detected
   1D1C 67            [ 4]  135         ld h, a                     ;; H is the mask
   1D1D 3A 24 1D      [13]  136         ld a, (_size_counter)
   1D20 6F            [ 4]  137         ld l, a                     ;; L is the offset
                            138         ; ld (_current_key_pressed), hl
   1D21 C9            [10]  139         ret
   1D22                     140 _no_key_detected:
   1D22 23            [ 6]  141     inc hl
                     0030   142 _size_counter = .+1
   1D23 3E 00         [ 7]  143     ld a, #0x00                     ;; AUTOMODIFIABLE, A = counter
   1D25 3C            [ 4]  144     inc a
   1D26 18 E8         [12]  145     jr _kp_loop
   1D28                     146 _kp_endLoop:
   1D28 21 00 00      [10]  147     ld hl, #0x00                    ;; Return 0 if no key is pressed
   1D2B 3E 00         [ 7]  148     ld a, #0
   1D2D 32 31 1D      [13]  149     ld (_key_released), a
   1D30 C9            [10]  150     ret
                            151 
   1D31                     152 _key_released:
   1D31 00                  153     .db #0
                            154 
                            155 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                            156 ;; DESCRIPTION
                            ...
                            170 ;;
   1D32                     171 sys_input_waitKeyPressed::
   1D32 CD 0C 1D      [17]  172     call sys_input_getKeyPressed
   1D35 3A 31 1D      [13]  173     ld a, (_key_released)
   1D38 B7            [ 4]  174     or a
   1D39 20 F7         [12]  175     jr nz, sys_input_waitKeyPressed
   1D3B AF            [ 4]  176     xor a
   1D3C B4            [ 4]  177     or h
   1D3D B5            [ 4]  178     or l
   1D3E 28 F2         [12]  179     jr z, sys_input_waitKeyPressed
   1D40 3E 01         [ 7]  180     ld a, #1
   1D42 32 31 1D      [13]  181     ld (_key_released), a
   1D45 C9            [10]  182     ret
                            183 
                            184 
                            185 ;;-----------------------------------------------------------------
...
Hexadecimal [16-Bits]
   ...
   1D46 C9            [10]  195     ret 
                            ...
                            205 ;;  Modified: iy, bc
                            206 ;;
   1D47                     207 sys_input_generic_update::
   1D47 18 05         [12]  208     jr first_key
   1D49                     209 keys_loop:
   1D49 01 04 00      [10]  210     ld bc, #4
   1D4C FD 09         [15]  211     add iy, bc
   1D4E                     212 first_key:
   1D4E FD 6E 00      [19]  213     ld l, 0(iy)                     ;; Lower part of the key pointer
   1D51 FD 66 01      [19]  214     ld h, 1(iy)                     ;; Lower part of the key pointer
                            215     ;; Check if key is null
   1D54 7D            [ 4]  216     ld a, l
   1D55 B4            [ 4]  217     or h
   1D56 C8            [11]  218     ret z                           ;; Return if key is null
                            219     ;; Check if key is pressed
   1D57 CD 98 1E      [17]  220     call cpct_isKeyPressed_asm      ;;
   1D5A 28 ED         [12]  221     jr z, keys_loop
                            222     ;; Key pressed execute action
   1D5C 21 49 1D      [10]  223     ld hl, #keys_loop               ;;
   1D5F E5            [11]  224     push hl                         ;; return addres from executed function
   1D60 FD 6E 02      [19]  225     ld l, 2(iy)                     ;;
   1D63 FD 66 03      [19]  226     ld h, 3(iy)                     ;; retrieve function address    
   1D66 E9            [ 4]  227     jp (hl)                         ;; jump to function
                            228 
                            229 
                            230 ;;-----------------------------------------------------------------
                            231 ;;
                            ...
                            237 ;;  Modified: iy, bc
                            238 ;;
   1D67                     239 sys_input_main_menu_update::
   1D67 FD 21 72 24   [14]  240     ld iy, #sys_input_main_menu_actions
   1D6B CD 47 1D      [17]  241     call sys_input_generic_update
   1D6E C9            [10]  242     ret
                            243 
...
Hexadecimal [16-Bits]
                            ...
                            251 ;;  Output:
                            252 ;;  Modified: iy, bc
   1D6F                     253 sys_input_player_left::
   1D6F DD 66 0F      [19]  254     ld h, e_vx(ix)
   1D72 DD 6E 10      [19]  255     ld l, e_vx+1(ix)
   1D75 01 18 00      [10]  256     ld bc, #STEP_HORIZONTAL_SPEED           
   1D78 B7            [ 4]  257     or a
   1D79 ED 42         [15]  258     sbc hl, bc                          ;; add STEP SPEED to current SPEED
   1D7B 01 00 FF      [10]  259     ld bc, #MAX_HORIZONTAL_SPEED_NEG    ;; check if MAX HORIZONTAL SPEED has been reached
   1D7E B7            [ 4]  260     or a                                ;;
   1D7F 22 87 1D      [16]  261     ld (sipl_max_not_reached+1), hl     ;; save new vx value for later use
   1D82 ED 42         [15]  262     sbc hl, bc                          ;;
   1D84 38 0B         [12]  263     jr c, sipl_max_reached              ;;
   1D86                     264 sipl_max_not_reached:
   1D86 21 00 00      [10]  265     ld hl, #00000
   1D89 DD 74 0F      [19]  266     ld e_vx(ix), h
   1D8C DD 75 10      [19]  267     ld e_vx+1(ix), l
   1D8F 18 06         [12]  268     jr sipl_exit
   1D91                     269 sipl_max_reached:                       ;; if max speed reached, vx set to max speed
   1D91 DD 70 0F      [19]  270     ld e_vx(ix), b
   1D94 DD 71 10      [19]  271     ld e_vx+1(ix), c
   1D97                     272 sipl_exit:
   1D97 C9            [10]  273     ret
                            274 
                            275 ;;-----------------------------------------------------------------
                            ...
                            282 ;;  Output:
                            283 ;;  Modified: iy, bc
   1D98                     284 sys_input_player_right::    
   1D98 DD 66 0F      [19]  285     ld h, e_vx(ix)
   1D9B DD 6E 10      [19]  286     ld l, e_vx+1(ix)
   1D9E 01 18 00      [10]  287     ld bc, #STEP_HORIZONTAL_SPEED           
   1DA1 ED 4A         [15]  288     adc hl, bc                          ;; add STEP SPEED to current SPEED
   1DA3 01 00 01      [10]  289     ld bc, #MAX_HORIZONTAL_SPEED_POS    ;; check if MAX HORIZONTAL SPEED has been reached
   1DA6 B7            [ 4]  290     or a                                ;;
   1DA7 22 AF 1D      [16]  291     ld (sipr_max_not_reached+1), hl     ;; save new vx value for later use
   1DAA ED 42         [15]  292     sbc hl, bc                          ;;
   1DAC 30 0B         [12]  293     jr nc, sipr_max_reached              ;;
   1DAE                     294 sipr_max_not_reached:
   1DAE 21 00 00      [10]  295     ld hl, #00000
   1DB1 DD 74 0F      [19]  296     ld e_vx(ix), h
   1DB4 DD 75 10      [19]  297     ld e_vx+1(ix), l
   1DB7 18 06         [12]  298     jr sipr_exit
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 111.
Hexadecimal [16-Bits]



   1DB9                     299 sipr_max_reached:                       ;; if max speed reached, vx set to max speed
   1DB9 DD 70 0F      [19]  300     ld e_vx(ix), b
   1DBC DD 71 10      [19]  301     ld e_vx+1(ix), c
   1DBF                     302 sipr_exit:
   1DBF C9            [10]  303     ret
                            304 
                            305 ;;-----------------------------------------------------------------
                            ...
                            312 ;;  Output:
                            313 ;;  Modified: iy, bc
   1DC0                     314 sys_input_player_up::
   1DC0 DD 66 11      [19]  315     ld h, e_vy(ix)
   1DC3 DD 6E 12      [19]  316     ld l, e_vy+1(ix)
   1DC6 01 30 00      [10]  317     ld bc, #STEP_VERTICAL_SPEED           
   1DC9 B7            [ 4]  318     or a
   1DCA ED 42         [15]  319     sbc hl, bc                          ;; add STEP SPEED to current SPEED
   1DCC 01 00 FE      [10]  320     ld bc, #MAX_VERTICAL_SPEED_NEG      ;; check if MAX HORIZONTAL SPEED has been reached
   1DCF B7            [ 4]  321     or a                                ;;
   1DD0 22 D8 1D      [16]  322     ld (sipu_max_not_reached+1), hl     ;; save new vx value for later use
   1DD3 ED 42         [15]  323     sbc hl, bc                          ;;
   1DD5 38 0B         [12]  324     jr c, sipu_max_reached              ;;
   1DD7                     325 sipu_max_not_reached:
   1DD7 21 00 00      [10]  326     ld hl, #00000
   1DDA DD 74 11      [19]  327     ld e_vy(ix), h
   1DDD DD 75 12      [19]  328     ld e_vy+1(ix), l
   1DE0 18 06         [12]  329     jr sipu_exit
   1DE2                     330 sipu_max_reached:                       ;; if max speed reached, vx set to max speed
   1DE2 DD 70 11      [19]  331     ld e_vy(ix), b
   1DE5 DD 71 12      [19]  332     ld e_vy+1(ix), c
   1DE8                     333 sipu_exit:
   1DE8 C9            [10]  334     ret
                            335 
                            336 ;;-----------------------------------------------------------------
                            ...
                            343 ;;  Output:
                            344 ;;  Modified: iy, bc
   1DE9                     345 sys_input_player_down::    
   1DE9 DD 66 11      [19]  346     ld h, e_vy(ix)
   1DEC DD 6E 12      [19]  347     ld l, e_vy+1(ix)
   1DEF 01 30 00      [10]  348     ld bc, #STEP_VERTICAL_SPEED           
   1DF2 ED 4A         [15]  349     adc hl, bc                          ;; add STEP SPEED to current SPEED
   1DF4 01 00 02      [10]  350     ld bc, #MAX_VERTICAL_SPEED_POS      ;; check if MAX HORIZONTAL SPEED has been reached
   1DF7 B7            [ 4]  351     or a                                ;;
   1DF8 22 00 1E      [16]  352     ld (sipd_max_not_reached+1), hl     ;; save new vx value for later use
   1DFB ED 42         [15]  353     sbc hl, bc                          ;;
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 112.
Hexadecimal [16-Bits]



   1DFD 30 0B         [12]  354     jr nc, sipd_max_reached              ;;
   1DFF                     355 sipd_max_not_reached:
   1DFF 21 00 00      [10]  356     ld hl, #nullptr
   1E02 DD 74 11      [19]  357     ld e_vy(ix), h
   1E05 DD 75 12      [19]  358     ld e_vy+1(ix), l
   1E08 18 06         [12]  359     jr sipd_exit
   1E0A                     360 sipd_max_reached:                       ;; if max speed reached, vx set to max speed
   1E0A DD 70 11      [19]  361     ld e_vy(ix), b
   1E0D DD 71 12      [19]  362     ld e_vy+1(ix), c
   1E10                     363 sipd_exit:
   1E10 C9            [10]  364     ret
                            365 
                            366 ;;-----------------------------------------------------------------
                            ...
                            373 ;;  Output:
                            374 ;;  Modified: iy, bc
   1E11                     375 sys_input_player_jump::
   1E11 DD 7E 1A      [19]  376     ld a, e_on_platform(ix)     ;; if not on platform don't jump
   1E14 B7            [ 4]  377     or a                        ;;
   1E15 C8            [11]  378     ret z                       ;;
                            379 
   1E16 DD 66 11      [19]  380     ld h, e_vy(ix)              ;; Jump subtracts 2 to vy
   1E19 DD 6E 12      [19]  381     ld l, e_vy+1(ix)            ;;
   1E1C 01 00 03      [10]  382     ld bc, #0x0300              ;;
   1E1F B7            [ 4]  383     or a                        ;;
   1E20 ED 42         [15]  384     sbc hl, bc                  ;;
   1E22 DD 74 11      [19]  385     ld e_vy(ix), h              ;;
   1E25 DD 75 12      [19]  386     ld e_vy+1(ix), l            ;;
   1E28 DD 36 1A 00   [19]  387     ld e_on_platform(ix), #0    ;; after jumping we are not on platform
   1E2C C9            [10]  388     ret
                            389 
                            390 
                            391 ;;-----------------------------------------------------------------
                            ...
                            398 ;;  Output:
                            399 ;;  Modified: iy, bc
   1E2D                     400 sys_input_player_update::
   1E2D DD E5         [15]  401     push ix                             ;; save ix information
   1E2F CD 8C 0A      [17]  402     call man_entity_getPlayerPosition   ;; move ix to the first entity which will be player operated
                            403 
   1E32 FD 21 74 24   [14]  404     ld iy, #sys_input_player_actions
   1E36 CD 47 1D      [17]  405     call sys_input_generic_update
   1E39 DD E1         [14]  406     pop ix                              ;; rerieve saved information in ix
   1E3B C9            [10]  407     ret
                            408 
