...
Hexadecimal [16-Bits]
   ...
   1BDF 3E 04         [ 7]   52     ld a, #e_cmpID_Collision
   1BE1 CD 9A 06      [17]   53     call man_components_getArrayHL
   1BE4 22 32 1C      [16]   54     ld  (_ent_array_ptr), hl
   1BE7 C9            [10]   55     ret
                             56 
                             57 
                             58 ;;-----------------------------------------------------------------
                             59 ;;
                             60 ;; sys_collision_check_collider_colisionable
                             61 ;;
   ...
   1BE8                      69 sys_collision_check_collider_colisionable::
                             70     ;; x axis collision
                             71     ;; case 1
   1BE8 FD 7E 05      [19]   72     ld a, e_x(iy)                   ;; a = iy.x
   1BEB 47            [ 4]   73     ld b, a                         ;; b = a = iy.x
   1BEC FD 86 09      [19]   74     add e_w(iy)                     ;; a = iy.x + iy.w
   1BEF 3D            [ 4]   75     dec a                           ;; a = iy.x + iy.w - 1
   1BF0 DD 4E 05      [19]   76     ld c, e_x(ix)                   ;; c = ix.x
   1BF3 91            [ 4]   77     sub c                           ;; a = iy.x + iy.w - 1 - ix.x
   1BF4 D8            [11]   78     ret c                           ;; return if no collision
...
Hexadecimal [16-Bits]
                             ...
                             79     ;; case 2
   1BF5 79            [ 4]   80     ld a, c                         ;; a = ix.x
   1BF6 DD 86 09      [19]   81     add e_w(ix)                     ;; a = ix.x + ix.w
   1BF9 3D            [ 4]   82     dec a                           ;; a = ix.x + ix.w - 1
   1BFA 90            [ 4]   83     sub b                           ;; a = iy.x + ix.w - 1 - iy.x
   1BFB D8            [11]   84     ret c                           ;; return if no collision
                             85 
                             86     ;; y axis collision
                             87     ;; case 3
   1BFC FD 7E 07      [19]   88     ld a, e_y(iy)                   ;; a = iy.y
   1BFF 47            [ 4]   89     ld b, a                         ;; b = a = iy.y
   1C00 FD 86 0A      [19]   90     add e_h(iy)                     ;; a = iy.y + iy.y
   1C03 3D            [ 4]   91     dec a                           ;; a = iy.y + iy.y - 1
   1C04 DD 4E 07      [19]   92     ld c, e_y(ix)                   ;; c = ix.y
   1C07 91            [ 4]   93     sub c                           ;; a = iy.y + iy.y - 1 - ix.y
   1C08 D8            [11]   94     ret c                           ;; return if no collision
                             95     ;; case 4
   1C09 79            [ 4]   96     ld a, c                         ;; a = ix.y
   1C0A DD 86 0A      [19]   97     add e_h(ix)                     ;; a = ix.y + ix.y
   1C0D 3D            [ 4]   98     dec a                           ;; a = ix.y + ix.y - 1
   1C0E 90            [ 4]   99     sub b                           ;; a = iy.y + ix.y - 1 - iy.y
                            100     ;;ret c                           ;; return if no collision - unncessary
   1C0F C9            [10]  101     ret
                            102 
                            103 ;;-----------------------------------------------------------------
                            104 ;;
                            105 ;; sys_collision_update_one_entity
                            106 ;;
                            107 ;;  Initilizes render system
                            108 ;;  Input: ix : pointer to the entity
                            109 ;;  Output: 
                            110 ;;  Modified: AF, BC, HL
                            111 ;;
   1C10                     112 sys_collision_collider_colisionable::
   1C10 CD E8 1B      [17]  113     call sys_collision_check_collider_colisionable  ;; check if there is a collision
   1C13 D8            [11]  114     ret c                                           ;; return if there is no collisión between ix and iy
                            115 
                            116     ;; change x velocity because of the collision
   1C14 FD 66 0F      [19]  117     ld h, e_vx(iy)
   1C17 FD 6E 10      [19]  118     ld l, e_vx+1(iy)
   1C1A CD A9 0C      [17]  119     call sys_util_negHL
   1C1D FD 74 0F      [19]  120     ld e_vx(iy), h
   1C20 FD 75 10      [19]  121     ld e_vx+1(iy), l
   1C23 FD 36 1F 01   [19]  122     ld e_moved(iy), #1
                            123 
   1C27 C9            [10]  124     ret
                            125 
                            126 
                            127 ;;-----------------------------------------------------------------
                            128 ;;
                            129 ;; sys_collision_update_one_entity
                            130 ;;
                            131 ;;  Initilizes render system
                            132 ;;  Input: ix : pointer to the entity
                            133 ;;  Output: 
...
Hexadecimal [16-Bits]
                            ...
                            134 ;;  Modified: AF, BC, HL
                            135 ;;
   1C28                     136 sys_collision_update_one_collider::
   1C28 21 10 1C      [10]  137     ld hl, #sys_collision_collider_colisionable
   1C2B 06 80         [ 7]  138     ld b, #e_cmp_collisionable
   1C2D CD 89 09      [17]  139     call man_entity_forall_matching_iy
   1C30 C9            [10]  140     ret
                            141 
                            142 ;;-----------------------------------------------------------------
                            143 ;;
                            144 ;; sys_collision_update
                            145 ;;
                            146 ;;  Initilizes collision system
                            147 ;;  Input: 
                            148 ;;  Output: 
                            149 ;;  Modified: AF, BC, DE, HL
                            150 ;;
                            151 
   1C31                     152 sys_collision_update::
                            153 
                     0053   154 _ent_array_ptr = . + 1
   1C31 21 00 00      [10]  155     ld  hl, #0x0000
                            156 
   1C34                     157     _loop:
                            158     ;;  Select the pointer to the entity with collision and prepare the next position for the next iteration.
   1C34 5E            [ 7]  159     ld e, (hl)
   1C35 23            [ 6]  160     inc hl
   1C36 56            [ 7]  161     ld d, (hl)
   1C37 23            [ 6]  162     inc hl
                            163 
                            164     ;;  The entities are finished traversing when find a pointer to null.
   1C38 7B            [ 4]  165     ld a, e
   1C39 B2            [ 4]  166     or d
   1C3A C8            [11]  167     ret z
                            168 
   1C3B E5            [11]  169     push hl
                            170 
   005D                     171     ld__ixl_e
   1C3C DD 6B                 1    .dw #0x6BDD  ;; Opcode for ld ixl, e
   005F                     172     ld__ixh_d
   1C3E DD 62                 1    .dw #0x62DD  ;; Opcode for ld ixh, d
                            173 
   1C40 CD 28 1C      [17]  174     call sys_collision_update_one_collider
                            175 
   1C43 E1            [10]  176 	pop hl
                            177 
   1C44 18 EE         [12]  178     jr _loop
                            179 
   1C46 C9            [10]  180     ret
